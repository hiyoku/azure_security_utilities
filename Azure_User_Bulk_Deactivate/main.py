...
FILENAME_USERS = "Azure_User_Bulk_Deactivate/deactivate_list.txt"
...
def main():
    ...
    with open(FILENAME_USERS, mode="r", encoding="utf8") as remove_file:
        for line_id in remove_file.readlines():
            ...
            print(f"Disabling ID: {_user_id} - {user_info['displayName']}... ", end="")
            res = az_cli(f"ad user update --id {_user_id} --account-enabled false")
